...
#: Нижняя планка задержки перед ответом. Комментарий через полсекунды
#: после поста не пишет ни один человек, и это первое, за что
#: комментатора считают роботом.
COMMENT_MIN_DELAY = max(5, _int("COMMENT_MIN_DELAY", "15"))
