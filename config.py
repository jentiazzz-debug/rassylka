...
#: Ник бота без «собаки». Нужен страницам документов: они открываются
#: в браузере, и с них должен быть путь обратно в сервис — иначе
#: страница выглядит ничьей. Узнаётся при старте у самого Telegram, если
#: не задан руками.
...
#: Ключ Anthropic. Пусто — радар поднимется, но будет работать одним
#: предфильтром: ключевые слова без разбора смысла. Это честнее, чем
#: падать на старте, — предфильтр сам по себе отсекает основной мусор,
#: просто пропускает больше ложных.
ANTHROPIC_API_KEY = (os.getenv("ANTHROPIC_API_KEY") or "").strip()

#: Модель классификатора.
#:
#: По умолчанию стоит старшая: качество разбора «заказ или болтовня»
#: прямо определяет, сколько мусора человек прочитает руками. Но поток
#: здесь оптовый — тысячи сообщений в сутки на тенанта, — и это ровно
#: та задача, где claude-haiku-4-5 дешевле в пять раз и почти наверняка
#: достаточен. Менять осознанно и после замера на своих чатах: цена
#: ошибки — не деньги, а пропущенный заказ.
RADAR_MODEL = (os.getenv("RADAR_MODEL") or "claude-opus-5").strip()

#: Сколько сообщений уходит в один запрос к модели. Разбор по одному
#: оплачивал бы системный промпт на каждом сообщении; складывать больше
#: тоже плохо — пачка ждёт, пока наберётся, а радар продаётся скоростью.
RADAR_BATCH = max(1, min(20, _int("RADAR_BATCH", "8")))

#: Сколько секунд копить пачку, прежде чем отправить недобранной. В
#: тихом чате сообщения приходят по одному, и без этого потолка первое
#: из них ждало бы соседей до вечера.
RADAR_BATCH_WAIT = _float("RADAR_BATCH_WAIT", "2.0")

#: Опрос страховкой — как у автокомментариев и по той же причине:
#: подключение рвётся, процесс перезапускается, событие теряется.
RADAR_POLL = max(15, _int("RADAR_POLL", "60"))

#: Сколько пропущенных сообщений добирать из одного чата за раз. В
#: живом чате за час набегают сотни, и гнать их все через модель после
#: перезапуска — это счёт за мусор.
RADAR_CATCHUP = max(1, _int("RADAR_CATCHUP", "40"))

#: Границы длины сообщения-кандидата. Короче — почти всегда реплика в
#: беседе («да», «+», «сколько?»), длиннее — статья или реклама.
RADAR_MIN_LEN = max(1, _int("RADAR_MIN_LEN", "40"))
RADAR_MAX_LEN = max(RADAR_MIN_LEN, _int("RADAR_MAX_LEN", "2500"))

#: Порог, ниже которого лид не показывается. Модель возвращает оценку
#: 0..100; всё, что ниже, тихо оседает в базе — посмотреть можно, но в
#: личку оно не придёт.
RADAR_MIN_SCORE = max(0, min(100, _int("RADAR_MIN_SCORE", "55")))

#: Сколько радаров можно завести одному человеку и сколько чатов в
#: каждом. Потолок на чаты не бюрократия: каждый чат — это поток в
#: классификатор, то есть прямые деньги.
MAX_RADARS = max(1, _int("MAX_RADARS", "3"))
MAX_RADAR_CHATS = max(1, _int("MAX_RADAR_CHATS", "30"))

#: Суточный потолок обращений к модели на одного человека. Упёрлись —
#: радар не глохнет, а переходит на один предфильтр и говорит об этом.
RADAR_DAILY_CALLS = max(1, _int("RADAR_DAILY_CALLS", "400"))


# --- поддержка --------------------------------------------------------

#: Сколько обращений в сутки может завести один человек. Не от жадности:
...
TICKETS_PER_DAY = max(1, _int("TICKETS_PER_DAY", "2"))
...
def legal_ready() -> bool:
    """Есть ли путь в поддержку со страниц документов.

    Путь один и он всегда на месте — обращение в приложении, — поэтому
    страницам ничего не нужно от настроек. Функция осталась ради тех
    мест, что её вызывают, и ради дня, когда касса потребует вернуть на
    страницу почту.
    """
    return True
...
def check() -> None:
    ...
    if not webapp_ready():
        log.warning(
            "без WEBAPP_URL в поддержку не написать: обращения идут "
